...
export default function SolvingTestPage() {
  ...
  // Czy bieżące pytanie jest w trybie "czytanie + odpowiedź" (TimedReveal)?
  const isTimedReveal =
    !!currentQuestion &&
    !Array.isArray(currentQuestion.extra_data) &&
    currentQuestion.extra_data?.timed_reveal === true &&
    (currentQuestion.type === "SingleChoice" ||
      currentQuestion.type === "MultipleChoice");

  // W fazie czytania (odliczanie jeszcze trwa) blokujemy przycisk "Dalej".
  const inReadingPhase =
    isTimedReveal && !revealedQuestions.has(currentQuestion.question_id);

  const markRevealed = (qid) =>
  ...
  const renderAnswerComponent = (extraProps = {}) => {
    const props = {
  ...
  const renderQuestion = () => {
    ...
    if (isTimedReveal) {
      const delay = Number(currentQuestion.extra_data?.reveal_delay) || 3;
      const answerLimit =
        Number(currentQuestion.extra_data?.answer_time_limit) || 0;
      return (
        <TimedRevealGate
          key={currentQuestion.question_id}
          question={currentQuestion}
          revealDelay={delay}
          answerTimeLimit={answerLimit}
          alreadyRevealed={revealedQuestions.has(currentQuestion.question_id)}
          onRevealComplete={markRevealed}
          onAnswerTimeout={() => goToNextQuestion()}
        >
          {renderAnswerComponent({ hideQuestionText: true })}
        </TimedRevealGate>
      );
    }

    return renderAnswerComponent();
  };
  ...
  return (
    <div className="mx-auto bg-gradient-to-br from-white via-indigo-100 to-white dark:bg-gradient-to-br dark:from-[#141517] dark:via-[#1d212a] dark:to-[#131314] p-5 pt-15 md:pt-25 font-sans flex flex-col justify-start items-center min-h-screen min-w-[320px] overflow-x-hidden">
      {warning.visible && (
      ...
      {!warning.visible && currentQuestion && (
        <>
          <div className="fixed top-0 left-1/2 transform -translate-x-1/2 flex justify-between items-center w-[90%] sm:w-[85%] lg:w-[95%] mt-6 sm:mt-8 text-center">
            ...
            >
              ...
              <div className="flex justify-between items-center mt-6">
                ...
                <button
                  ...
                  disabled={isTestFinished || inReadingPhase}
