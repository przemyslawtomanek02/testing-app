...
function CreateTestsPanel({ setOverlayImage }) {
  ...
  const addNewQuestion = (type = "SingleChoice") => {
    const defaultQuestion = JSON.parse(
      JSON.stringify(QuestionDefaults[type] || QuestionDefaults.SingleChoice),
    );
    ...
    reader.onload = (e) => {
      try {
        ...
        if (!jsonData.test_name || !Array.isArray(jsonData.questions)) {
          toast.error(
            "Invalid JSON structure. Required fields: 'test_name' and 'questions' array.",
          );
          return;
        }
        ...
        const validatedQuestions = jsonData.questions.map((q) => {
          const type = q.type;
          const defaults =
            QuestionDefaults[type] || QuestionDefaults.SingleChoice;
  ...
  const handleImageUpload = useCallback(
    (qIndex, file) => {
      ...
      if (file.size > maxSizeMB * 1024 * 1024) {
        toast.error(
          `Image for question ${qIndex + 1} is too large. Max size: ${maxSizeMB}MB.`,
        );
        return;
      }
      setImages((prevImages) => {
  ...
  const validateQuestions = (questions) => {
    ...
    questions.forEach((q, index) => {
      ...
      switch (q.type) {
        case "FillInTheBlank": {
          const allAnswersHaveText = q.answers?.every(
          ...
          if (
            definedAnswersCount > 0 &&
            definedAnswersCount !== placedBlanksCount
          ) {
            errors.push(
              `${prefix}: number of gaps ≠ number of responses (${placedBlanksCount} ≠ ${definedAnswersCount}).`,
            );
            hasError = true;
          }
          break;
        ...
        case "MatchingMultiple": {
          const allFieldsFilled = q.answers?.every(
            (a) =>
              typeof a.left?.text === "string" &&
              typeof a.right?.text === "string" &&
              a.left.text.trim().length > 0 &&
              a.right.text.trim().length > 0,
          );
          if (!allFieldsFilled) {
            errors.push(
              `${prefix}: Each match must have “left” and “right” fields.".`,
            );
            hasError = true;
          }
          break;
    ...
    <div className="p-4 sm:p-6 md:p-8 bg-slate-50 dark:bg-darkCustom-800 font-sans min-h-screen">
      <form onSubmit={handleSubmit(onSubmit)} className="mx-auto h-full">
        <div className="flex flex-wrap items-center justify-between gap-4 mb-8">
          <h1 className="text-3xl font-bold text-slate-800 dark:text-darkCustom-50">
            Create Test
          </h1>
        ...
        {jsonFileName && (
          <p className="text-sm text-slate-600 dark:text-darkCustom-300 mb-4 -mt-4">
            Imported file: {jsonFileName}
          </p>
        )}

        <div className="bg-white dark:bg-darkCustom-900 rounded-lg shadow-md border border-slate-200 dark:border-darkCustom-700 h-full">
          ...
          <div className="p-6 divide-y divide-gray-300 dark:divide-darkCustom-700 space-y-6">
            ...
            <div className="pt-6">
              ...
              {!validateQuestions(watchedQuestions).valid && (
                <p className="text-slate-500 dark:text-darkCustom-400 text-xs mt-2">
                  Please fill all fields and place all answers before adding a
                  new question.
                </p>
              )}
            </div>
